...
import com.yango.packj.contract.TstInV6;
import com.yango.packj.contract.TstOutV6;
import com.yango.packj.contract.TstPayV6;
...
import com.yango.packj.client.Utils.ErrCode;
...
public class PosClient {
    ...
    public DeployRtn deployContractPosIn(String loc) {
        try {
            ...
            DeployRtn rtn = new DeployRtn(Utils.ErrCode.OK,pIn.getContractAddress());
            return rtn;
        } catch (Exception e) {
            //System.out.println("Deploy contract TstIn, failed " + e.getMessage());
            DeployRtn rtn  = new DeployRtn(Utils.ErrCode.FAIL, "");

            return rtn;
        }
    }

    public Utils.ErrCode InsertPosIn(String contractAddress, String berthId, long inTime, int inTimeType, int inType, String plateId, int prepayLen, int prepayMoney, int vehicleType, String inPicHash) {
        try {
            ...
            if (!response.isEmpty()) {
                if (response.get(0).ret.compareTo(new BigInteger("0")) == 0) {
                    ...
                    return Utils.ErrCode.OK;
                } else {
                    System.out.printf(" insert record failed from %s \n", response.get(0).account);
                    return Utils.ErrCode.OPERATION_FAIL;
                }
            } else {
